...
const FarmerDetailsCard = ({ farmerDetails }: FarmerDetailsCardProps) => {
  ...
  return (
    <aside className="flex h-full min-h-130 w-full flex-col overflow-hidden rounded-[30px] border border-green-100 bg-white shadow-sm transition-colors duration-200 dark:border-[#29463a] dark:bg-[#141c23]">
      {/* Header */}
      <div className="relative shrink-0 bg-linear-to-br from-green-600 to-emerald-500 px-6 py-7 text-white">
        <div className="relative flex flex-col items-center text-center">
          <div className="h-24 w-24 overflow-hidden rounded-full border-4 border-white bg-green-100 shadow-lg dark:bg-[#123126]">
            {farmerDetails.profilePicture ? (
              <img
                src={farmerDetails.profilePicture}
                alt={farmerDetails.username}
          ...
          <h2 className="mt-3 wrap-break-word text-xl font-bold">
            {farmerDetails.username || "Farmer"}
          </h2>
        </div>
      </div>
      ...
      <div className="flex flex-1 flex-col justify-between p-5">
        <div className="space-y-3">
          ...
          {farmerDetails.bio && (
            <div className="rounded-2xl bg-green-50 p-4 transition-colors duration-200 dark:bg-[#123126]">
              <div className="mb-2 flex items-center gap-2 font-semibold text-green-700 dark:text-[#00c767]">
                <Sprout size={18} />
                About Farmer
              </div>

              <p className="line-clamp-3 wrap-break-word text-sm leading-6 text-gray-600 dark:text-[#9aa7b1]">
                {farmerDetails.bio}
              </p>
            </div>
          )}
...
const InfoRow = ({
  ...
  return (
    <div className="flex min-w-0 items-start gap-3 rounded-2xl border border-gray-100 bg-gray-50 p-3 transition-colors duration-200 dark:border-[#29343c] dark:bg-[#192128]">
      <div className="shrink-0 rounded-xl bg-green-100 p-2 text-green-700 dark:bg-[#123126] dark:text-[#00c767]">
        {icon}
      </div>

      <div className="min-w-0 flex-1">
        <p className="text-xs font-semibold uppercase text-gray-400 dark:text-[#71808a]">
          {label}
        </p>

        <p className="mt-1 wrap-break-word text-sm font-semibold text-gray-800 dark:text-[#f5f7f8]">
          {value}
        </p>
      </div>
    </div>
